...
function zoneAnswerZonesHtml(s,l,publicView){
  const physical=["A","B","C","D"],layout=zoneStudentLayout(s,l),correct=l.reveal?String(l.correct||"").toUpperCase():"",picked=l.quizMode==="peace"&&s&&s.liveAnswer?zoneLetterAt(s.liveAnswer.x,s.liveAnswer.y):"";
...
function bindZoneCountdown(){
  ...
  const tick=()=>{const cur=state.lesson;if(!cur||!cur.active||cur.questionId!==l.questionId){clearInterval(zoneCountdownTimer);return;}const now=Date.now(),elapsed=now-(cur.startedAt||now),preparing=now<(cur.readyUntil||0);let countText="";if(elapsed<1000)countText="3";else if(elapsed<2000)countText="2";else if(elapsed<3000)countText="1";else if(elapsed<4000)countText="GO!";document.querySelectorAll(".zone-start-countdown").forEach(el=>{el.textContent=countText;el.classList.toggle("show",!!countText);el.classList.toggle("go",countText==="GO!");});const total=Math.max(1000,(cur.durationSec||10)*1000),left=preparing?total:Math.max(0,(cur.endsAt||0)-now),pct=preparing?100:Math.max(0,Math.min(100,left/total*100)),fill=pct>55?"#ffd234":pct>25?"#ff8b2d":"#ef3838";document.querySelectorAll(".zone-timer>i").forEach(el=>{el.style.width=pct+"%";el.style.backgroundColor=fill;});document.querySelectorAll(".zone-timer>span").forEach(el=>el.textContent=preparing?"準備！":(left/1000).toFixed(1)+" 秒");document.querySelectorAll(".zone-submit-progress").forEach(el=>{el.style.setProperty("--quiz-time",pct+"%");el.style.setProperty("--quiz-fill",fill);});if(!preparing&&view.page==="teacher"&&!cur.settled)maybeAutoSettleConfirmedZone();if(!preparing&&left<=0){clearInterval(zoneCountdownTimer);if(view.page==="teacher"&&cur.quizMode!=="offline"&&!cur.settled){settleZoneByCurrentPositions("⏰ 時間到，已依最後所在區域自動結算");return;}cur.locked=true;save();render();toast("⏰ 時間到，答案已鎖定！");}};
...
function maybeAutoSettleConfirmedZone(){
  const l=state.lesson,now=Date.now();if(view.page!=="teacher"||!l||!l.active||l.mode!=="zone"||l.quizMode==="offline"||l.locked||l.settled||now<(l.autoSettleAfter||l.readyUntil||0))return false;
  const joined=state.students.filter(s=>{const z=s.liveAnswer||{};return z.questionId===l.questionId&&Number(z.joinedAt)>=Number(l.startedAt||0)&&Number(z.joinedAt)<=Number(l.autoSettleAfter||l.endsAt||Infinity);});
...
function zoneBoardHtml(){
  ...
  const joinedCount=state.students.filter(s=>{const z=s.liveAnswer||{};return z.questionId===l.questionId&&Number(z.joinedAt)>=Number(l.startedAt||0);}).length;
  ...
  return '<div class="zone-quiz-shell"><div class="zone-question'+(l.visualSvg?' has-geometry':'')+'"><b>'+esc(l.prompt||l.title)+'</b>'+quizGeometryHtml(l.visualSvg,"zone-geometry")+quizImageHtml(l.questionImage,"zone-question-img","題目圖片")+answerReveal+'<div class="zone-timer"><span>'+timerText+' 秒</span></div></div>'
    ...
    +offlineAnswerBoardHtml(l)+'<div class="zone-quiz-controls"><span class="mini">'+zoneModeLabel(l.quizMode)+'・'+(l.locked?'🔒 已鎖定':'🟢 作答中')+'・已進場 '+joinedCount+' 人・已確認 '+counts.reduce((a,b)=>a+b,0)+' / '+Math.max(joinedCount,1)+'</span>'
...
function zoneStudentHtml(s,l){
  ...
  const status=zoneStudentStatusText(z,l),peace=l.quizMode==="peace";
  ...
  return '<div class="panel zone-student"><h3>🧭 '+esc(l.title||"角色站位答題")+'・'+zoneModeLabel(l.quizMode)+'</h3><div class="zone-touch-guide">'+(peace?'👆 直接點選四個答案區；選好後再按下確定答案。':'☝️ 請按住場地並滑動角色；單點不會移動。確認答案後即鎖定，不能再更改位置。')+'</div>'
    +'<div class="zone-student-map'+(peace?' peace-select':'')+(z.confirmed?' answer-locked':'')+'"><div class="zone-cross-v"></div><div class="zone-cross-h"></div>'+zones+zoneObjectsHtml(s,l)+'<div class="zone-wait">待命</div><div class="zone-avatar '+(z.confirmed?'confirmed ':'')+(ko?'zone-ko':'')+(streak>=3?' zone-fever':'')+(streak>=10?' zone-lightning':'')+'" style="left:'+z.x+'%;top:'+z.y+'%;--fever-speed:'+Math.max(.24,1.1-Math.min(streak,12)*.07)+'s">'+dollSVG(s,72)+'<span class="zone-hp"><i style="width:'+hp+'%"></i></span><span class="name">'+esc(s.name)+' HP '+Math.round(z.quizHp)+(streak>=3?' 🔥'+streak:'')+'</span></div><div class="zone-start-countdown">3</div>'+(l.buzzerWinner?'<div class="zone-buzzer-winner">🚨 '+esc(l.buzzerWinner.name)+' 搶答</div>':'')+'</div>'
    ...
    +(l.quizMode==="buzzer"?'<div class="zone-confirm"><div class="zone-submit-actions"><button class="btn zone-buzz-button zone-submit-progress" id="zoneBuzz" style="--quiz-time:100%"'+(!z.answer||l.locked||ko||preparing?' disabled':'')+'><span class="zone-submit-label">🚨 搶答！'+(z.answer?'（'+z.answer+'）':'')+'</span></button><button class="btn zone-recover" id="zoneRecover" title="操作卡住時重新載入本題，不會清除位置或答案">↻</button></div></div>':'<div class="zone-confirm"><div class="zone-submit-actions"><button class="btn gold zone-submit-progress" id="zoneConfirm" style="--quiz-time:100%"'+(!z.answer||l.locked||preparing||z.confirmed?' disabled':'')+'>'+zoneConfirmButtonContent(z,l)+'</button><button class="btn zone-recover" id="zoneRecover" title="操作卡住時重新載入本題，不會清除位置或答案">↻</button></div></div>')+'<div class="mini" style="text-align:center;margin-top:8px">'+(l.quizMode==="buzzer"?'站好答案區後按搶答；第一位成功者會鎖定全場。':peace?'點選答案區後仍須按確定；送出後不能更改答案。':'送出前請確認位置；按下確定後，本題不能再移動或更改答案。')+'</div></div>';
}

function zoneStudentStatusText(z,l){return l.settled?(z.answer===l.correct?'🎉 答對了！':'📖 正解是 '+l.correct+'，一起訂正吧'):(z.quizHp<=0?(z.answer?'💫 體力歸零，固定在 '+z.answer+' 區，可確認目前答案':'💫 倒在中央待命區，本題無法作答'):(z.confirmed?'✅ 已確認 '+z.answer+'，等待老師公布':z.answer?(l.quizMode==="peace"?'已選擇 '+z.answer+'，請按下確定答案':'目前位於 '+z.answer+' 區，請確認答案'):(l.quizMode==="peace"?'請點選一個答案區':'位於中央待命區，請移動到答案區')));}
...
function paintZonePointerState(s,map){
  ...
  const name=avatar.querySelector(".name"),nameText=s.name+" HP "+Math.round(z.quizHp)+((Number(s.quizFeverStreak)||0)>=3?" 🔥"+s.quizFeverStreak:"");if(name&&name.textContent!==nameText)name.textContent=nameText;
  const status=document.querySelector(".student-zone-screen .zone-status"),statusText=zoneStudentStatusText(z,l);if(status&&status.textContent!==statusText)status.textContent=statusText;
  const usedSig=Object.keys(z.used||{}).sort().join("|");if(map._zoneUsedSig!==usedSig){map._zoneUsedSig=usedSig;map.querySelectorAll("[data-zone-object]").forEach(el=>{if(z.used&&z.used[el.dataset.zoneObject]&&!el.classList.contains("wall"))el.remove();});}
  if(l.quizMode==="peace"){const physical=zoneLetterAt(z.x,z.y).toLowerCase();map.querySelectorAll(".zone-area").forEach(el=>el.classList.toggle("peace-picked",!!physical&&el.classList.contains(physical)));}
  const confirm=document.getElementById("zoneConfirm");if(confirm){const disabled=!z.answer||l.locked||z.confirmed||Date.now()<(l.readyUntil||0),html=zoneConfirmButtonContent(z,l),sig=(disabled?"1":"0")+"|"+html;if(confirm._zoneSig!==sig){confirm._zoneSig=sig;confirm.disabled=disabled;confirm.innerHTML=html;}if(z.eventUntil>Date.now()&&map._zoneNoticeUntil!==z.eventUntil){map._zoneNoticeUntil=z.eventUntil;clearTimeout(map._zoneNoticeTimer);map._zoneNoticeTimer=setTimeout(()=>{map._zoneNoticeUntil=0;if(document.body.contains(map))paintZonePointerState(s,map);},Math.max(60,z.eventUntil-Date.now()+30));}}
  const buzz=document.getElementById("zoneBuzz");if(buzz){const disabled=!z.answer||l.locked||z.quizHp<=0||Date.now()<(l.readyUntil||0),text="🚨 搶答！"+(z.answer?"（"+z.answer+"）":"");if(buzz.disabled!==disabled)buzz.disabled=disabled;if(buzz.textContent!==text)buzz.textContent=text;}
}

function bindPeaceZoneSelection(s){
  const map=document.querySelector(".student-zone-screen .zone-student-map");if(!map)return;
  map.addEventListener("click",e=>{e.preventDefault();const l=state.lesson,z=zoneAnswerState(s,l),now=Date.now();if(!l||l.quizMode!=="peace"||l.locked||z.confirmed)return;if(now<(l.readyUntil||0)){toast("準備倒數中，GO！之後再選答案",true);return;}const r=map.getBoundingClientRect(),x=(e.clientX-r.left)/Math.max(1,r.width)*100,y=(e.clientY-r.top)/Math.max(1,r.height)*100,physical=zoneLetterAt(x,y),centers={A:[25,23],B:[75,23],C:[25,77],D:[75,77]};if(!physical||!centers[physical]){toast("請直接點選四個答案框",true);return;}z.x=centers[physical][0];z.y=centers[physical][1];z.answer=zoneLogicalAnswer(s,l,z.x,z.y);z.updatedAt=now;paintZonePointerState(s,map);syncZoneStudent(s,false);});
}
...
function renderStudentZoneInterface(s,l){
  const joined=zoneAnswerState(s,l);if(!joined.joinedAt){joined.joinedAt=Date.now();joined.updatedAt=joined.joinedAt;syncZoneStudent(s,true);}
  ...
  bindZoneCountdown();bindZoneGestureLock();if(l.quizMode==="peace")bindPeaceZoneSelection(s);else bindZonePointerMovement(s);
