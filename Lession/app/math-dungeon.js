...
function classroomApplyLaunch(){
  ...
  if(Array.isArray(d.questionBank)&&d.questionBank.length){
    ...
    const selectedBank=d.questionBank.filter(q=>(!q.grade||!grade||String(q.grade)===grade)&&(!Number(q.vol)||Number(q.vol)<=gradeMax)).slice(0,160).map((q,qIndex)=>{
      ...
      return visualizeGeometryQuestion(classroomNumberVariant({q:String(q.q||'請看圖片選擇正確答案'),opts,ans:String(q.ans||opts[0]||''),sol:String(q.sol||''),tag:[source,unit,topic].filter(Boolean).join(' · '),chap,unit,topic,scopeKey:courseScopeKey(chap,unit),source:q.source==='teacher'?'teacher':'course',grade:String(q.grade||grade),v,vol:v,fig:pics.join(''),difficulty:q.difficulty||q.diff||q.tier||''},v,qIndex));
